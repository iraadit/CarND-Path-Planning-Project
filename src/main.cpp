...
// For converting back and forth between mph and mp/s.
constexpr double miph4meps() { return 2.23694; }
double miph2meps(double x) { return x / miph4meps(); }
double meps2miph(double x) { return x * miph4meps(); }
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            //lane = getLane(car_d); // to consider the lane as the real position, and not the desired one; more dynamic (but too much, can cause lateral jerk)
            ...
            trajectory_planner(car_ahead, car_ahead_speed, ref_vel, speed_diff, map_waypoints_s, map_waypoints_x, map_waypoints_y, lane, car_s, car_x, car_y, car_yaw, prev_size, previous_path_x, previous_path_y, next_x_vals, next_y_vals);
